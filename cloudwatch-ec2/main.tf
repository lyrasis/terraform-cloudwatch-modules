...
resource "aws_cloudwatch_metric_alarm" "cpu" {
  ...
  period              = lookup(var.cpu_usage, "period", 60)
  evaluation_periods  = lookup(var.cpu_usage, "evaluation_periods", 60)
  datapoints_to_alarm = lookup(var.cpu_usage, "evaluation_datapoints", 45)
  metric_name         = "CPUUtilization"
  namespace           = "AWS/EC2"
  statistic           = lookup(var.cpu_usage, "statistic", "Average")
  threshold           = lookup(var.cpu_usage, "threshold", 80)
  unit                = "Percent"
  dimensions = {
...
resource "aws_cloudwatch_metric_alarm" "surplus-credit" {
  ...
  period              = lookup(var.credit_charge, "period", 300)
  evaluation_periods  = lookup(var.credit_charge, "evaluation_periods", 1)
  datapoints_to_alarm = lookup(var.credit_charge, "evaluation_datapoints", null)
  namespace           = "AWS/EC2"
  metric_name         = "CPUSurplusCreditsCharged"
  statistic           = lookup(var.credit_charge, "statistic", "Average")
  threshold           = lookup(var.credit_charge, "threshold", 0)
  unit                = "Count"
  dimensions = {
    InstanceId = var.instances[each.key].id
  }
